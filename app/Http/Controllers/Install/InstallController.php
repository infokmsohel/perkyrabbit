...
class InstallController extends Controller
{
    ...
    public function postDetails(Request $request)
    {
        ...
        try {
            ...
            /*
            $check = $this->checkEnvato($input['ENVATO_PURCHASE_CODE']);
            if(!is_bool($check)){
                return $check;
            }
...
*/
    ...
    {
        $url = 'https://ai.envato.com/v3/market/author/sale?code=' . $purchase_code;
        $curl = curl_init($url);
        ...
        if (isset($envatoRes->item->id)) {
            if ($this->envatoProductCode == $envatoRes->item->id) {
                return true;
            } else {
                return false;
            }
