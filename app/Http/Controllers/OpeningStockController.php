                    ...
                    if (array_key_exists($location_id, $locations)) {
                        ...
                        foreach ($value as $k => $rows) {
                            foreach ($rows as $key => $v) {
                                ...
                                //if ($qty > 0) {
                                $qty_remain_formated = $this->productUtil->num_f($qty_remaining);

                                if (isset($v['purchase_line_id'])) {
                                    $purchase_line = PurchaseLine::findOrFail($v['purchase_line_id']);
                                    //Quantity = remaining + used
                                    $qty_formated = $qty_remain_formated + $purchase_line->quantity_used;

                                    if($qty_formated != 0){
                                        //Calculate transaction total
                                        $purchase_total += ($purchase_price_inc_tax * $qty_formated);
                                    ...
                                    $qty_formated = $qty_remain_formated;
                                    if($qty_formated != 0){
                                ...
                                $purchase_line->quantity = $qty_formated;
