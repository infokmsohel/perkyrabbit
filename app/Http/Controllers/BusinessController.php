...
class BusinessController extends Controller
{
    ...
    */
     ...
     * @return void
     */
    public function __construct(BusinessUtil $businessUtil, RestaurantUtil $restaurantUtil, ModuleUtil $moduleUtil,AccountController $account)
    {
        $this->businessUtil = $businessUtil;
        $this->moduleUtil = $moduleUtil;
        $this->accountController = $account;
        $this->avlble_modules = [
            'tables' => [ 'name' =>'restaurant.tables',
                        'tooltip' => __('restaurant.tooltip_tables')
                    ] ,
            'modifiers' => [ 'name' =>'restaurant.modifiers',
                    'tooltip' => __('restaurant.tooltip_modifiers')
                ],
            'service_staff' => [
                    'name' => 'restaurant.service_staff',
                    'tooltip' => __('restaurant.tooltip_service_staff')
                ],
            'kitchen' => [
                'name' => 'restaurant.kitchen_for_restaurant'
            ],
            'account' => ['name' =>'lang_v1.account'],
            'subscription' => ['name' =>'lang_v1.enable_subscription']
        ];
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            ...
            $text = explode('@', $request->email);
            $emailpart = strtolower($text[1]);
            if($emailpart != "hisabe.net"){
                $output = ['success' => 0,
                    'msg' =>'please use hisabe.net email Or Contact with us.'
                ];
            ...
            $dir = "uploads/business/".str_replace(' ','_',$request->name).'/logo/';
            //$this->CheckDir($dir);
            $logo_name = $this->UploadImage($request,'business_logo',$dir,'400','320','');          
            $business_details['logo'] = $logo_name;
            ...
            //create new account
//            $users=$user->id;
//            $businesss=$business->id;
//            $newLocation=$new_location->id;
            $lName='';
            $this->accountController->storeAccount($user->id,$business->id,$new_location->id,$lName);
             ...
             *------------------------------------------------------------------- 
             */
            $this->SendConfirmationMail($request->email);
    ...
    /*
     * ---------------------------------------------------------------------
     * Account Confirmation Mail Send Function
     * ---------------------------------------------------------------------
     */
    
    public function SendConfirmationMail($email){
        $token = str_random(32);
        Session::put('email-verification-token',$token);
        $data = array(
            'email' => $email,
            'subject' => 'Email Verification',
            'messagebody' => 'Welcome to '.env('APP_TITLE').'<br> Please Verify your email to click on verification button.This link is valid for 30 mins.',
            'link' => env('APP_URL').'/email/verification?securityToken='.$token,
        );
        Mail::send('emails.accountConfirmation',$data,function($message) use($data){
    ...
    public function VerifyRegisterEmail(Request $request) {
        ...
        if(isset(Auth::user()->id)){
            if(Auth::user()->accountConfirmation ==1){
                return redirect('/home');
            }
            if(Session::get('email-verification-token') == $request->securityToken){
                $user = User::find(Auth::user()->id);
                $user->accountConfirmation = 1;
                $user->save();
                Session::forget('email-verification-token');
                return redirect('/home');                
            }else{
                return redirect('email/not-verified')->with('message','Security token is not match.');
 
            }           
        }else{
            return redirect('email/not-verified')->with('message','Confirmation Link is Expire');
        }
    ...
    /*
     * ----------------------------------------------------------------------------
     * Pending Verification Page Show
     * ----------------------------------------------------------------------------
     */
    public function PendingVerificationPageShow() {
        return view('others.emailConfirmationPage');
    }
    
    public function ResendConfirmationAgain() {
        if(Auth::user()->accountConfirmation ==1){
            return redirect('/home');
        }
        $this->SendConfirmationMail(Auth::user()->email);
        return back()->with('message','Verification link Resend Successfully');
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            //upload Business logo
            $dir = "uploads/business/". str_replace(' ','_',$business->name).'/logo/';            
            $logo_path = $this->UploadImage($request,'business_logo',$dir,'400','320',$business->logo);            
            $business_details['logo'] = $logo_path;
