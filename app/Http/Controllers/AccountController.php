...
//                                ->where('created_at', 'asc')
            ->select(['name','track_type', 'account_number', 'accounts.note', 'accounts.id',
...
//                                ->where('created_at', 'asc')
    ...
    public function create()
    {
        ...
        $newAccountTypes = Account::newAccountTypes();

        return view('account.create')
                ->with(compact('account_types','newAccountTypes'));
    }


    public function storeAccount($users,$businesss,$newLocation,$lName){

        $account= new Account();
        $account->business_id = $businesss;
        $account->name = empty($lName)?'Cash':'Cash-'.$lName;
        $account->account_number = $businesss.'-'.$newLocation.'-'.'1001';
        $account->account_type = "saving_current";
        $account->track_no = $businesss.$newLocation;
        $account->track_type = 'own';
        $account->note = 'Default Business Account';
        $account->created_by = $users;
        $account->save();
...
//    public function storeAccount($user, $business){
//
//        $accNo=rand(111111,999999).'_'.$business->id;
//
//
...
//            'note'=>'default bussiness account',
//            'created_by'=>$user->id
//        ]);
//
//        $opening_bal = 00;
