...
class CashRegisterUtil extends Util
{
        ...
        if ($status_before == 'draft' && $transaction->status == 'final') {
            ...
            $prev_payments = CashRegisterTransaction::where('transaction_id', $transaction->id)
                            ->select(
                                ...
                                DB::raw("SUM(IF(pay_method='emi', IF(type='emi', amount, -1 * amount), 0)) as total_emi"),
                                DB::raw("SUM(IF(pay_method='bkash', IF(type='bkash', amount, -1 * amount), 0)) as total_bkash"),
            ...
            if (!empty($prev_payments)) {
                $payment_diffs = [
                    ...
                    'bkash' => $prev_payments->total_bkash,
                    'emi' => $prev_payments->total_emi,
    ...
    public function refundSell($transaction)
    {
        ...
        $refunds = [
                    ...
                    'bkash' => $total_payment->total_bkash,
                    'emi' => $total_payment->total_emi,
    ...
    public function getRegisterDetails($register_id = null)
    {
        ...
        $register_details = $query->select(
            ...
            DB::raw("SUM(IF(pay_method='emi', IF(transaction_type='sell', amount, 0), 0)) as total_emi"),
            DB::raw("SUM(IF(pay_method='bkash', IF(transaction_type='sell', amount, 0), 0)) as total_bkash"),
            ...
            DB::raw("SUM(IF(transaction_type='refund', IF(pay_method='bkash', amount, 0), 0)) as total_bkash_refund"),
            DB::raw("SUM(IF(transaction_type='refund', IF(pay_method='emi', amount, 0), 0)) as total_emi_refund"),
