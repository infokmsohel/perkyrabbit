...
class TransactionUtil extends Util
{
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if($il->show_logo != 0){
            ...
            }elseif(!empty($il->logo) && file_exists($il->logo)){
                $output['logo'] = asset($il->logo);
            }
            else{
                if(file_exists($business_details->logo)){
                    $output['logo'] = asset($business_details->logo);
                }
            }
        }else{
             $output['logo'] = false;
        ...
        //$output['logo'] = $il->show_logo != 0 && ) ? asset('uploads/invoice_logos/' . $il->logo) : false;
        ...
        $output['customer_fatherName'] = '';
        $output['customer_mobileNo'] = '';
        ...
        if ($il->show_customer == 1) {
            ...
            $output['customer_mobileNo'] = $customer->mobile;
            $output['customer_fatherName'] = $customer->fatherName;
        ...
        if ($transaction_type == 'sell_return') {
            ...
            $output['total'] = $this->num_f($transaction->final_total, $show_currency, $business_details);
            $output['total_amount'] = $this->num_f($transaction->final_total);
            ...
            if ($il->show_payments == 1) {
                ...
                if (!empty($payments)) {
                    foreach ($payments as $value) {
                        ...
                        }elseif ($value['method'] == 'bkash') {
                            $output['payments'][] =
                                ['method' => trans("lang_v1.bkash") . (!empty($value['phone_number']) ? (', Phone Number:' . $value['phone_number']) : ''),
                                'amount' => $this->num_f($value['amount'], $show_currency, $business_details),
                                'date' => $this->format_date($value['paid_on'], false, $business_details)
                                ];
                        }elseif ($value['method'] == 'emi') {
                            $output['payments'][] =
                                ['method' => trans("lang_v1.emi") . (!empty($value['emi_no']) ? (', Emi_Number:' . $value['emi_no']) : ''),
                                'amount' => $this->num_f($value['amount'], $show_currency, $business_details),'depositor_name'=>$value['depositor_name'],
                                'date' => $this->format_date($value['paid_on'], false, $business_details)
                                ];
