		...
		<div class="{{$col_class}}">
			<div class="form-group">
				...
				<div class="input-group">
					...
					<select name="payment[{{$row_index}}][account_id]" id="account_{{$row_index}}" class="form-control">
						@foreach($accountsinfo as $acc)
							<option value="{{$acc->id}}" {{$ownAcc->id == $acc->id ?'selected':''}} >{{$acc->name}}</option>
						@endforeach
					</select>
{{--					{!! Form::select("payment[$row_index][account_id]", $accounts, !empty($payment_line['account_id']) ? $payment_line['account_id'] : 'selected' , ['class' => 'form-control select2', 'id' => "account_$row_index", 'style' => 'width:100%;']); !!}--}}
				</div>
			</div>
		</div>
	@endif
