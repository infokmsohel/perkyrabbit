  ...
  <div class="box box-solid">
    <div class="box-body">
      <div class="row">
        ...
        <div class="col-sm-4">
          <div class="form-group">
           {!! Form::label('productType ', __('lang_v1.select_product_type').' :*') !!}
            <select name="productType" class="form-control" required>
                <option value="0" {{$product->productType == 0?'selected':''}}>Others Product</option>
                <option value="1" {{$product->productType == 1?'selected':''}}>Vehicle</option>
            </select>
          </div>
        </div>
        <div class="col-sm-12 text-right">
            <button type="button" class="btn btn-info " data-toggle="modal" data-target="#add_custom_field_name">{{__('lang_v1.add_custom_field_name')}}</button>            
            <button type="button" class="btn btn-info " id="create_custom_field">{{__('lang_v1.create_custom_field')}}</button>
            <button type="button" class="btn btn-primary" id="remove_custom_field" onclick="removeCustom()"> {{__('lang_v1.remove_custom_field')}} </button>
        </div>
        <div class="clearfix"></div>
        <fieldset>
            <input type="hidden" name="customFieldCount" value="{{count($added_custom_field)}}" id="count">
            <?php $i = 0;?>
            @foreach($added_custom_field as $field)
            <?php $i++;?>
            <div class="form-group" id="customblock{{$i}}">
                <div class="col-xs-6 col-md-6">
                    <label>{{__('lang_v1.select_custom_field')}} </label>
                    <select name="customField{{$i}}" required class="form-control" placeholder="{{__('lang_v1.select_custom_field')}}">
                        @foreach($all_custom_fields as $cfields)
                        <option value="{{$cfields->id}}" {{$cfields->id == $field->custonFieldId?'selected':''}} >{{$cfields->customFieldName}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-xs-6 col-md-6">
                    <label>{{__('lang_v1.custom_field_value')}}*</label>
                    <input type="text" value="{{$field->customFieldValue}}" name="customFieldValue{{$i}}" class="form-control" placeholder="{{__('lang_v1.custom_field_value')}}" required>
                </div>
            </div>             
            @endforeach
        </fieldset>
...
<!-- /.content -->
@include('product.partials.addCustomeField')
@endsection
...
@section('javascript')
  ...
  <script>
        ...
        var i = $('#count').val();
        if(i>0){
            $('#remove_custom_field').show();
        }
        var x = document.getElementById('count');
        $(document).on("click", "#create_custom_field", function(){  
            i++;
        ...
        <div class="form-group" id="customblock'+i+'">\n\
            <div class="col-xs-6 col-md-6">\n\
                <label>{{__('lang_v1.select_custom_field')}}</label>\n\
                <select name="customField'+i+'" required class="form-control" placeholder="{{__('lang_v1.select_custom_field')}}">\n\
                    <option value="" selected>Select Custom Field</option>@foreach($all_custom_fields as $field)<option value="{{$field->id}}">{{$field->customFieldName}}</option>@endforeach\n\
                </select>\n\
            </div>\n\
            <div class="col-xs-6 col-md-6">\n\
                <label>{{__('lang_v1.custom_field_value')}}*</label>\n\
                <input type="text" name="customFieldValue'+i+'" class="form-control" placeholder="{{__('lang_v1.custom_field_value')}}" required>\n\
            </div>\n\
        </div>');
        x.value=i;
        $('#remove_custom_field').show();
