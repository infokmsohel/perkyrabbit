      ...
      <ul class="sidebar-menu">
          ...
          @if(in_array('account', $enabled_modules))
            <li class="treeview {{ $request->segment(1) == 'account' ? 'active active-sub' : '' }}">
              ...
              <ul class="treeview-menu">
                  ...
                  <li class="{{ $request->segment(1) == 'account' && $request->segment(2) == 'payment-account-report' ? 'active' : '' }}"><a href="{{action('AdvancePaymentController@ShowAdvncePaymentPage')}}"><i class="fa fa-file-text-o"></i>{{__('account.advance_payment')}}</a></li>
              </ul>
            </li>
          @endif
        @endcan
        ...
        @if(auth()->user()->can('purchase_n_sell_report.view') 
          ...
          <li class="treeview {{  in_array( $request->segment(1), ['reports']) ? 'active active-sub' : '' }}" id="tour_step8">
            ...
            <ul class="treeview-menu">
                <li ><a href="{{action('CustomPaymentController@ShowDetails')}}"><i class="fa fa-money"></i>{{__('account.custom_money_receipt')}}
